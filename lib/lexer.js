/**************************************** LEXER ************************************************
 * The lexer class is an interator that takes a String as input in Lexer.input
...
Lexer.prototype.next = function () {
    ...
    if (this.from >= this.buflen) return null;

    //Always add cases in descending order of occurrence probability
    if      (this._processIdentifier()) { return this._consume(TokenTypes.IDENTIFIER) }
    else if (this._processOperator())   { return this._consume(TokenTypes.OPERATOR);  }
    else if (this._processNumber())     { return this._consume(TokenTypes.NUMBER);    }
    else if (this._processComment())    { return this._consume(TokenTypes.COMMENT);   }
    else                                { return this._processError();                }
};
...
Lexer.prototype._make = function (type, value) {
    return {type:type, value:value, from:this.from, to:this.i}
};
...
Lexer.prototype._processSingleLineComment = function (chars) {
    if (chars == "//") {
        while (this.from < this.buflen && !_isNewline(this.buf.charAt(this.i))) {
            this.i++;
        }
        this.i++;
        return true
...
Lexer.prototype._processMultiLineComment = function (chars) {
    if (chars == "/*") {
        this.i = this.i + 2;
        while (this.from < this.buflen && this.buf.charAt(this.i) != '*' && this.buf.charAt(this.i) != '/') {
            this.i++;
        }
        this.i = this.i + 2;
        return true;
