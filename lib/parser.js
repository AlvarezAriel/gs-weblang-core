function Parser(lexer) {
    ...
    this.advance = function (id) {
        ...
        if (id && this.token.id !== id) {
            this.error(this.token, "Expected '" + id + "'.");
        }
        if (!tokens.hasNext()) {
    ...
    this.assignment = function (id) {
        return self.infixr(id, 10, function (left) {
            if (left.id !== "." && left.id !== "[" && left.arity !== "name") {
                self.error(left, "Bad lvalue.");
            }
            this.left = left;
            this.right = self.expression(9);
    ...
    this.statements = function () {
        ...
        return statementsList.length === 1 ? statementsList[0] : statementsList;
    };
